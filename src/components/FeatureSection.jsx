...
import DraftPetitions from '../assets/svgs/draft petitions.svg';
import InstantLegalChat from '../assets/svgs/instant legal chat.svg';
import LegalTemplates from '../assets/svgs/legal templates.svg';
...
const FeatureSection = () => (
	<Container>
		<section className="w-full flex flex-col items-center py-16 bg-gradient-to-b from-[#F5F7FA] to-[#C3CFE2]">
			...
			<div className="grid grid-cols-1 md:grid-cols-3 gap-y-16 gap-x-20 md:gap-x-[80px] w-full max-w-6xl px-2 sm:px-4">
				{features.map((feature, idx) => (
					<div
						key={idx}
						className="bg-white/10 border border-white/30 rounded-[8px] p-4 sm:p-6 flex flex-col justify-between text-center min-h-[220px] min-w-[160px] w-full md:w-[360px] md:h-[300px] shadow-lg backdrop-blur relative"
						style={{ margin: '0 0 48px 0' }}
					>
						<div className="flex items-center justify-between w-full mb-2 sm:mb-4">
							<h3 className="font-['Plus Jakarta Sans'] font-medium text-[22px] sm:text-[26px] md:text-[30px] leading-[28px] sm:leading-[32px] md:leading-[34.6px] tracking-[-1.26px] text-black text-left">
								{feature.title}
							</h3>
							<img src={feature.icon} alt="icon" className="w-[20px] h-[20px] sm:w-[23px] sm:h-[23px] ml-2 object-contain" />
						</div>
